...
import {
  User, Package, MapPin, Truck, LogOut, CheckCircle,
      ...
      <div className="min-h-screen flex items-center justify-center pt-20" style={{ background:C.ivory }}>
        ...
        >
          <div className="w-16 h-16 rounded-full mx-auto mb-5 flex items-center justify-center"
            style={{ background:`${C.maroon}18` }}>
            <User size={28} style={{ color:C.maroon }} />
          </div>
          <h2 className="text-3xl font-bold mb-3" style={{ fontFamily:serif, color:C.maroon }}>Sign In Required</h2>
          <p className="text-sm mb-8" style={{ color:C.muted }}>Please sign in to view your account.</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => router.push("/login")} className="gold-btn px-6 py-3 rounded-xl text-sm">Sign In</button>
            <button onClick={() => router.push("/signup")}
              className="px-6 py-3 rounded-xl text-sm font-bold transition-all"
              style={{ border:`1.5px solid ${C.maroon}`, color:C.maroon }}
              onMouseEnter={e=>(e.currentTarget.style.background=`${C.maroon}0e`)}
      ...
      <div style={{ background:`linear-gradient(135deg,${C.deepBurg},${C.burgundy})` }}>
        <div className="max-w-7xl mx-auto px-5 py-10 flex items-center gap-5">
          ...
          <motion.div initial={{ opacity:0, x:16 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.2 }}>
            <h1 className="text-2xl font-bold" style={{ fontFamily:serif, color:C.ivory }}>Welcome, {user.name.split(" ")[0]}</h1>
            <p className="text-sm" style={{ color:`${C.ivory}60` }}>{user.email}</p>
          </motion.div>
          <div className="ml-auto flex items-center gap-4">
            ...
            <button onClick={() => { logout(); router.push("/"); }}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold transition-all"
              style={{ background:`rgba(212,24,61,0.12)`, border:`1px solid rgba(212,24,61,0.3)`, color:"#FF6B80" }}>
